...
class Manager:
    ...
    def __init__(self, master):
        ...
        self.btn_fs_service = Button(master, text="Start", command=self.start_fs)
        
        self.lbl_tn = Label(master, text="Tunnel")
        self.status_tn = Label(master, text="not running")
        self.btn_tn_service = Button(master, text="Start", command=self.start_tn)
    ...
    def start_fs(self):
        if self._running_fs:
            ...
            self.proc_fs = Popen(['tpi-fileserver'],stdout=PIPE,stdin=PIPE,shell=True)
    ...
    def start_tn(self):
        if self._running_tn:
            ...
            self.proc_tn = Popen(['tpi-tunnel'],stdout=PIPE,stdin=PIPE,shell=True)
...
def handleLogin(*args):
    ...
    call(['taskkill', '/F', '/T', '/PID',  str(proc_fs.pid)],stdout=PIPE,stdin=PIPE)
